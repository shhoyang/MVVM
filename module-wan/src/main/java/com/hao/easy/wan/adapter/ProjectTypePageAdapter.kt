...
class ProjectTypePageAdapter :
    ...
    override fun bindViewHolder(
        ...
        viewHolder.viewBinding {
            val innerAdapter = ProjectTypeItemAdapter()
            root.init(innerAdapter, 4)
            innerAdapter.resetData(item)
        }
    }
}

class ProjectTypeItemAdapter :
    BaseNormalAdapter<WanItemProjectTypeBinding, ProjectType>() {
